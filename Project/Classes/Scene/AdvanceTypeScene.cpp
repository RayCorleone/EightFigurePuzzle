...
Block* block3[N][N];				// 二维向量,九宫格
int a3[N][N][maxState] = { 0 };	// 状态记录列表
int path3[maxState] = { 0 };
int shortest3[maxState] = { 0 };
int now3[N][N] = { 0 };
int nex3[N][N] = { 0 };
int cnt3 = 0;

bool ready1 = 0;
std::string s1;		//起始状态字符
bool sOk1 = 0;	//起始状态输入结束
std::string t1;		//终止状态字符
bool tOk1 = 0;	//目标状态输入结束

Scene* AdvanceTypeScene::createScene()
{
	return AdvanceTypeScene::create();
}

bool AdvanceTypeScene::init()
{
...
//静态元素
void AdvanceTypeScene::initStillNode()
{
...
//动态元素
void AdvanceTypeScene::initMoveLayer()
{
	auto vSize = Director::getInstance()->getVisibleSize();
	Vec2 origin = Director::getInstance()->getVisibleOrigin();
	...
	auto startBottom = MenuItemImage::create("pic/YesNormal.png", "pic/YesSelected.png",
		CC_CALLBACK_1(AdvanceTypeScene::startEdit, this, startEditBox));
	...
	auto targetBottom = MenuItemImage::create("pic/YesNormal.png", "pic/YesSelected.png",
		CC_CALLBACK_1(AdvanceTypeScene::targetEdit, this, targetEditBox));
	...
	auto pauseItem = MenuItemImage::create("pic/PauseNormal.png", "pic/PauseSelected.png",
		CC_CALLBACK_1(AdvanceTypeScene::getSetUpLayer, this, moveLayer));
...
//菜单元素
void AdvanceTypeScene::initMenu(Node* pNode)
{
	auto vSize = Director::getInstance()->getVisibleSize();
	Vec2 origin = Director::getInstance()->getVisibleOrigin();
	...
	musicSlider->addTargetWithActionForControlEvents(this,
		cccontrol_selector(AdvanceTypeScene::musicSliderCallBack), Control::EventType::VALUE_CHANGED);
	...
	effectSlider->addTargetWithActionForControlEvents(this,
		cccontrol_selector(AdvanceTypeScene::effectSliderCallBack), Control::EventType::VALUE_CHANGED);
	pNode->addChild(effectSlider, 6);

	//动态菜单关闭按钮
	auto menuCloseItem = MenuItemImage::create("pic/MenuCloseNormal.png", "pic/MenuCloseSelected.png",
		CC_CALLBACK_1(AdvanceTypeScene::menuClose, this, pNode));
	...
	auto returnGameItem = MenuItemImage::create("pic/ReturnHomeNormal.png", "pic/ReturnHomeSelected.png",
		CC_CALLBACK_1(AdvanceTypeScene::returnCallBack, this));
	...
	auto closeItem = MenuItemImage::create("pic/CloseNormal2.png", "pic/CloseSelected2.png",
		CC_CALLBACK_1(AdvanceTypeScene::gameCloseCallback, this));
...
//音乐滑块效果
void AdvanceTypeScene::musicSliderCallBack(Ref* pSender, Control::EventType type)
{
...
//音效滑块效果
void AdvanceTypeScene::effectSliderCallBack(Ref* pSender, Control::EventType type)
{
	auto slider = (ControlSlider*)pSender;
	auto value = slider->getValue();
	value = value / 100;
...
//获取起始状态
void AdvanceTypeScene::startEdit(Ref* pSender, EditBox* editbox)
{
	//输入合法标志
	bool f = 0;
	bool List[9] = { 0 };
	...
	if (sOk1 == 0) {	//还未初始化
		...
		if (f == 1)	sOk1 = 0;	//输入不合法，重新输入
		else		sOk1 = 1;	//输入合法，不再输入
	}
}

//获取终止状态
void AdvanceTypeScene::targetEdit(Ref* pSender, EditBox* editbox)
{
	//输入合法标志
	bool f = 0;
	bool List[9] = { 0 };
	...
	if (tOk1 == 0) {	//还未初始化
		...
		if (f == 1)	tOk1 = 0;	//输入不合法，重新输入
		else		tOk1 = 1;	//输入合法，不再输入
	}
}

//鼠标点击关闭操作
void AdvanceTypeScene::gameCloseCallback(Ref* pSender)
{
...
//鼠标点击弹出菜单
void AdvanceTypeScene::getSetUpLayer(Ref* pSender, Node* pNode)
{
...
//鼠标点击关闭菜单
void AdvanceTypeScene::menuClose(Ref* pSender, Node* pNode)
{
...
//返回上级场景
void AdvanceTypeScene::returnCallBack(Ref* pSender)
{
	step = 0;
	maxStep = 0;
	isEnd = 0;
...
/*********************************演示逻辑部分*********************************/
void AdvanceTypeScene::initGameLayer()
{
	//数据输入
	this->schedule(schedule_selector(AdvanceTypeScene::initData), 0.05f);
	this->schedule(schedule_selector(AdvanceTypeScene::moveBlocks), 1.5f);
	this->schedule(schedule_selector(AdvanceTypeScene::endGame), 2.0f);
	this->schedule(schedule_selector(AdvanceTypeScene::stepUpdate), 0.05f);
}

void AdvanceTypeScene::initData(float dt) {
	...
	if (sOk1 == 1 && tOk1 == 1 && ready1 == 0) {	// 输入均正确
		//步数栏
		...
		for (int i = 0; i < N; i++) {
			for (int j = 0; j < N; j++) {
				start[i][j] = s1[i * N + j] - '0';
				target[i][j] = t1[i * N + j] - '0';
			}
		}
		statecpy(now3, start);
		statecpy(nex3, start);
		...
		// A*寻路
		int res = Astar2(a3, start, target, path3);
		if (res == -1) {
			isEnd = 1;
			maxStep = maxState;
		}
...
void AdvanceTypeScene::stepUpdate(float dt)
{
	if (ready1 == 1) {
		char newStep[20];
		if (step == maxStep) {
...
void AdvanceTypeScene::moveBlocks(float dt)
{
	if (ready1 == 1) {
		if (cnt3 == maxStep) {	// 刚开始无操作
			step = 0;
			cnt3 = cnt3 - 1;
		}
		else if (cnt3 >= 0) {
			...
			int x1 = 0, y1 = 0;	//交换位置
			getBrack(now3, x0, y0);
			getState(a3, nex3, shortest3[cnt3]);
			getBrack(nex3, x1, y1);

			// 开始移动
			cocos2d::MoveTo* move1 = cocos2d::MoveTo::create(0.5f, block3[x0][y0]->getPos());
			cocos2d::MoveTo* move2 = cocos2d::MoveTo::create(0.5f, block3[x1][y1]->getPos());

			auto effectID = CocosDenshion::SimpleAudioEngine::sharedEngine()->playEffect("audio/move.mp3");
			block3[x0][y0]->getPic()->runAction(Sequence::create(move2->clone(), DelayTime::create(0.05f), NULL));
			block3[x1][y1]->getPic()->runAction(Sequence::create(move1->clone(), DelayTime::create(0.05f), NULL));

			Point po = block3[x0][y0]->getPos();
			block3[x0][y0]->setPos(block3[x1][y1]->getPos());
			block3[x1][y1]->setPos(po);
			std::swap(block3[x0][y0], block3[x1][y1]);
...
void AdvanceTypeScene::endGame(float dt)
{
	if (ready1 == 1) {
		if (isEnd == 1 && step == maxStep) {	//全部走完
			initEndLayer(1);
			this->unschedule(schedule_selector(AdvanceTypeScene::endGame));
			this->unschedule(schedule_selector(AdvanceTypeScene::moveBlocks));
		}
...
void AdvanceTypeScene::initEndLayer(const int result)
{
	auto vSize = Director::getInstance()->getVisibleSize();
	Vec2 origin = Director::getInstance()->getVisibleOrigin();
